...
const MerchantAddOrder = () => {
  ...
  const [selectboxPickupArea, setSelectboxPickupArea] = useState([])
  ...
  useEffect(() => {
    const subscription = watch((value, { name, type }) => {
      ...
      if (name == "pickup_city" && type == "change") {
        // setSelectboxPickupCity([])
        fetchPickupAreaData(value.pickup_city.value)
      }
  ...
  const fetchPickupAreaData = (productTypeId) => {
    ...
    return useJwt
      ...
      .then((res) => {
        ...
        setSelectboxPickupArea(areaData)
        return res.data
      })
      .catch((err) => console.log(err))
  }
