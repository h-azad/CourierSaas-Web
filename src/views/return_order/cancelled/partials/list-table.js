...
//           useJwt
//             .axiosPost(
//               getApi(`${CREATE_ORDER_LIST}${info.id}/cancel_order/`),
//               { details: info }
//             ) 
//             .then((res) => {
...
//     return useJwt
//       // .axiosGet(getApi(ORDER_PICKUP_FAILED_LIST) + `?page=${pageNumber}`)
//       .axiosGet(getApi(ORDER_PICKUP_FAILED_LIST) + `?${qs.stringify(filterQuery)}`)
//       .then((res) => {
...
//     return useJwt
//       .axiosGet(getApi(ORDER_PICKUP_FAILED_LIST) + '?' + searchTerm)
//       .then((res) => {
...
//                       <DropdownItem href="/" onClick={e => orderCancelled(e, info)}>
...
const CreateOrderList = () => {
  ...
  const assignHandler = (e) => {
    ...
    enterLoading(true)
    useJwt
      .axiosPost(getApi(CREATE_ORDER_DETAILS) + `${selectedInfo.id}/pickup_assign_to_rider/`, { selectedRiderIds: selectedRiderIds })
      .then((res) => {
        enterLoading(false)
        setStatusModalState(false)
